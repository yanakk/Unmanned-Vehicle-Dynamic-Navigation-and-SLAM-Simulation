...
# complete this node, and show the route on rviz
...
class GlobalPlanner:
    def __init__(self):
        ...
        self.plan_robot_radius = 0.6    # 0.6
        self.plan_ox = []
        self.plan_oy = []
        self.plan_rx = []
        self.plan_ry = []
        ...
        self.replan_sub = rospy.Subscriber('/collision_checker_result',Bool,self.replanCallback)    # try rp
        self.map_sub = rospy.Subscriber('/grid_map_mine', OccupancyGrid, self.NewupdateMap)
    ...
    def replanCallback(self, boolmsg):
        print('get CChecker msg ', boolmsg.data)
        if boolmsg.data:
            self.replan(0)
    ...
    def replan(self,req):
        ...
        self.updateGlobalPose()
        self.plan_rx,self.plan_ry = self.a_star.planning(self.plan_sx,self.plan_sy,self.plan_gx,self.plan_gy)   # get route from astar, input start, end goals.
        self.publishPath()
        res = True
        return PlanResponse(res)

    def updateMap(self):
        rospy.wait_for_service('/static_map')   # '/static_map'
        try:
        ...
        ox,oy = np.nonzero(map_data > 50)   # >50 consider as ob
        ...
        self.a_star = AStarPlanner(self.plan_ox,self.plan_oy,self.plan_grid_size,self.plan_robot_radius)  # input obstacles and grid, radius
    
    def NewupdateMap(self, msg):
        self.map = msg
        # Update for planning algorithm
        map_data = np.array(self.map.data).reshape((-1,self.map.info.height)).transpose()
        ox,oy = np.nonzero(map_data > 60)   # >50 consider as ob
        self.plan_ox = (ox*self.map.info.resolution+self.map.info.origin.position.x).tolist()
        self.plan_oy = (oy*self.map.info.resolution+self.map.info.origin.position.y).tolist()
        self.a_star = AStarPlanner(self.plan_ox,self.plan_oy,self.plan_grid_size,self.plan_robot_radius)  # input obstacles and grid, radius
    ...
    def publishPath(self):
        ...
        self.path_pub.publish(path)         # publish
