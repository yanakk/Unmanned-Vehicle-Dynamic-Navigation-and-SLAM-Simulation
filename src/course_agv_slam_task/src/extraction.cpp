...
public:
    ...
    // filter for landmarks extractions , if it is a cylinder
    float radius_max_th;
    // filter for landmarks extractions,    if <, not landmark
    int landMark_min_pt;
...
{   
    ...
    {   
        ...
        curr_xy << input.ranges[i] * std::cos(angle), input.ranges[i] * std::sin(angle);    // current point
        
        delta_dis = this->calc_dist(curr_xy, last_xy);

        if(delta_dis > range_threshold) // dis > threshold, new cluster
            label++;

        laser_extr.intensities[i] = label;  // label starts from 0
...
{   
    int total_num = (input.angle_max - input.angle_min) / input.angle_increment + 1;    // total point number

    LandMarkSet landMarks;

    // TODO: please code by yourself-----------------------
    // extract landmark from already labeled laser values      input.intensities[i] = label
    /*typedef struct{
    std::vector<float> position_x;
    std::vector<float> position_y;
    std::vector<int> id;
    ...
    int label_num = input.intensities[total_num-1];
    //cout<<"Verify label nums: "<<label_num<<endl;

    int label_cnt = 0;              // number in each label
    float x_mid, y_mid = 0;         // x,y average
    //float x[total_num], y[total_num] = 0;   // all of x,y in a label
    //float x_dis, y_dis = 0;
    float x_max, y_max = 0;
    float x_min, y_min = 50;

    Vector3d last_xy(input.ranges[0] * std::cos(input.angle_min), input.ranges[0] * std::sin(input.angle_min), input.intensities[0]);
    Vector3d curr_xy;    
    for (int i=0; i<total_num; i++){
        
        float angle = input.angle_min + i * input.angle_increment;
        curr_xy << input.ranges[i] * std::cos(angle), input.ranges[i] * std::sin(angle), input.intensities[i];    // current point
        
        if (curr_xy[2] == last_xy[2]){  // if label is same
            label_cnt++;
            x_mid += curr_xy[0];
            y_mid += curr_xy[1];
        ...
        else{               // fisrt make landmark, then save next label
            x_mid /= label_cnt;
            y_mid /= label_cnt;
            
            if ( (x_max-x_min < radius_max_th) && (y_max - y_min < radius_max_th) && (label_cnt >= landMark_min_pt) && (fabs(x_mid) < 4) && (fabs(y_mid) < 4))
            {
                landMarks.position_x.push_back(x_mid);
                landMarks.position_y.push_back(y_mid);
                landMarks.id.push_back(last_xy[2]); 
            }
            ...
            x_max = x_min = curr_xy[0];
            y_max = y_min = curr_xy[1];
        }
...
void extraction::publishLandMark(LandMarkSet input)
{
    if(input.id.size() <= 0)
        return;
    cout<<"LANDMARK NUMS: "<<input.id.size()<<endl;
