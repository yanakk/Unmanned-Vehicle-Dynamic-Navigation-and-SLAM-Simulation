...
public:
    ...
    //double mapgrid[map_width * map_height][2];
};
    ...
    n(n)
{
    ...
    n.getParam("/mapping/judge_ratio", judge_ratio);
    ...
    grid_points = Eigen::MatrixXd::Zero(map_width * map_height,5);  // x, y, num_ob, num_blank, ratio
    ...
    */
    ...
    for(size_t i = 0; i < laser_size; i ++)
    {
        ...
        if (R < range_max && R >= range_min)       // reach obstacles
            {
               for(double r = 0; r < R + cell_size; r += inc_step)  // 沿着激光射线以inc_step步进，更新地图
               {
                    ...
                    else if ( r > R + 0.5*cell_size && min < cell_size/2)   // obs
                    {
                        grid_points(id,2) += 1.0;
                        grid_points(id,4) = double(grid_points(id,2)/(grid_points(id,2) + grid_points(id,3)));
                    }
                        //grid_map.data[id] = 100;    // obstacle
...
void mapping::update_map(const double ratio, const int id)
{
